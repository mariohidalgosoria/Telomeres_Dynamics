...
int main(void){
  ...
  Count=0;
  ini  =  337;
  Nexp = 349;//number of time series
  dt=10;
  NC=(Nexp-ini)+1;
  R=2;
  
  sprintf(nam,"TA2CDT10C12.dat");//la cadena nom='V.dat'
 ...
 //get data from file CCLATA.dat
  j=0;
  sprintf(nom,"CCLAT%d.dat",A+ini-1);//coloca el id, file.dat en una cadena 'nom'
  ...
  j=0;
  //Captura de datos:
  sprintf(nom,"CCLAT%d.dat",A+ini-1);//coloca el id, xid.dat en una cadena 'nom'
...
//get data from file CCLATB.dat
  j=0;
  sprintf(nom,"CCLAT%d.dat",B+ini-1);//coloca el id, file.dat en una cadena 'nom'
  printf("%s\n",nom);//imprime en la pantalla file.dat
  tm0 = fopen(nom,"r");//abre file.dat para lectura, creando el puntero tm0
  while((!feof(tm0))){//mientras no sea el final de file.dat
    fscanf(tm0,"%lf",&Count);//lee el primer elemento de file.dat un float y la guarda en Count
  ...
  j=0;
  //Captura de datos:
  sprintf(nom,"CCLAT%d.dat",B+ini-1);//coloca el id, xid.dat en una cadena 'nom'
  printf("%s\n",nom);//imprime en la pantalla xid.dat
  tm0 = fopen(nom,"r");//abre xid.dat para lectura, creando el puntero tm0
  while((!feof(tm0))){//mientras no sea el final de xid.dat
    fscanf(tm0,"%lf",&Ser_t[j]);//lee el primer elemento de fileid.dat un float y la guarda en Ser_t[i]
  ...
  for(j=0;j <= Nds-12;j++){//X=[x0,x1,...,xNds-2] the loop has to go up to ((Nds-d)-2), to avoid evalution of Ser_X[Nds-1] and Ser_X[Nds]  
