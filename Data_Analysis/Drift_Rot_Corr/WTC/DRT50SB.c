/*Compile and run 8 times changing ini, Nexp, and l each time*/
#include<stdio.h>
#include<math.h>
#include<stdlib.h>
...
int main(void){
  ...
  //l=2;//use for LMNA1-3D_TEL.ods l=ini
  //l=27;//use for LMNA3-3D_TEL.ods l=ini
  //l=62;//use for LMNA4-3D_TEL.ods l=ini
  //l=100;//use for LMNA5-3D_TEL.ods l=ini
  //l=125;//use for LMNA6-3D_TEL.ods l=ini
  //l=160;//use for LMNA7-3D_TEL.ods l=ini
  //l=208;//use for LMNA9-3D_TEL.ods l=ini
  l=233;//use for LMNA10-3D_TEL.ods l=ini
  ...
  for(id=ini;id<=Nexp;id++){//for over each different file          
    ...
    i=0;
    sprintf(nom,"LAMT50T%d.dat",id);//coloca el id, xid.dat en una cadena 'nom'
  ...
  //for (i = 0; i < M; i++){
  //for (j = 0; j < N; j++)
  //  printf("%lf ", XT[i][j]);    // or *(A[r] + c) or *(*(A + r) + c)
  ...
  for(id=ini;id<=Nexp;id++){//for over each different file
    sprintf(nam,"CLAT50T%d.dat",id);//la cadena nom='V.dat'
