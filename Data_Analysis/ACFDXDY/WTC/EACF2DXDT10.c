...
int main(void){
  ...
  for(id=ini;id<=Nexp;id++){          
    ...
    i=0;
    //Captura de datos:
    sprintf(nom,"TAVDXDT10T%d.dat",id);//coloca el id, xid.dat en una cadena 'nom'
  ...
  sprintf(nom,"ETAVDXDT10.dat");//la cadena nom='x_mean.dat'
  tm0 = fopen(nom,"w"); //se crea el archivo x_mean.dat
      
  for(i=0;i< Ndat-1;i++){
    fprintf(tm0,"%lf  %lf\n",Ser_t[i],(Ser_X[i]/Nexp)/0.022415 );
  }
  printf("Numero de datos: %d\n ", i);
  fclose(tm0);     
  return 0;
