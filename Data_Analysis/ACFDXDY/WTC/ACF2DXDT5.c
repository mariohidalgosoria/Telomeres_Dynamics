...
int main(void){
  ...
  for(id=ini;id<=Nexp;id++){//for over each different file          
    ...
    sprintf(nam,"TAVDXDT5T%d.dat",id);//la cadena nom='V.dat'
	...
	//V[tau]+=  (Ser_X[i+tau+dt]*Ser_X[i+dt] - Ser_X[i+tau+dt]*Ser_X[i] - Ser_X[i+tau]*Ser_X[i+dt] + Ser_X[i+tau]*Ser_X[i])/(Ser_X[i+dt]*Ser_X[i+dt] - Ser_X[i+dt]*Ser_X[i] - Ser_X[i]*Ser_X[i+dt] + Ser_X[i]*Ser_X[i]);
	//printf("%d  %lf\n",d,V[d]);
      }
