...
int main(void){
  ...
  for(id=ini;id<=Nexp;id++){//for over each different file          
    ...
    while((!feof(tm0))){//mientras no sea el final de xid.dat
      ...
      //fscanf(tm0,"%lf",&Count);//lee el quinto elemento de xid.dat un float y la guarda en xser
      //        printf("%d\t%g\n",Ser_t[i],Ser_X[i]);
      i++;
    }// final del while
    fclose(tm0);
    ...
    //Nds Final time T
    for(i=0;i < (Nds-2);i++){//loop over time  	
      //theta = acos( (  (Ser_X[dt]-Ser_X[0]) * (Ser_X[i+2*dt]-Ser_X[i+dt] )   + (Ser_Y[dt]-Ser_Y[0]) * (Ser_Y[i+2*dt]-Ser_Y[i+dt] )    )  /  ( sqrt(pow(Ser_X[dt]-Ser_X[0],2)  + pow(Ser_Y[dt]-Ser_Y[0],2) )*sqrt(pow(Ser_X[i+2*dt]-Ser_X[i+dt],2)  + pow(Ser_Y[i+2*dt]-Ser_Y[i+dt],2) ) )  );
      //printf("%d  %lf\n",i,theta);
      theta = atan2((Ser_Y[i+dt]-Ser_Y[i]) , (Ser_X[i+dt]-Ser_X[i]));
      fprintf(tm1,"%lf\n",theta);
    }//end loop over time
