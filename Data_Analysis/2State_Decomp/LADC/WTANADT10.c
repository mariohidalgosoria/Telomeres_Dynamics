...
int main(void){
  ...
  sprintf(nam,"WTADT10.dat");//la cadena nom='V.dat'
  tm1 = fopen(nam,"w"); //se crea el archivo V.dat
  sprintf(nom,"WTNADT10.dat");//la cadena nom='V.dat'
  ...
  for(id=ini;id<=Nexp;id++){//for over each different file
    ...
    i=0;
    sprintf(nom,"THR10T%d.dat",id);//coloca el id, xid.dat en una cadena 'nom'
